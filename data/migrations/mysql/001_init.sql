-- MySQL schema — data/migrations/postgres/001_init.sql se wapas convert kiya gaya.
-- (Wo khud ek MySQL dump se bana tha, to ye asal shakl ke kareeb hai.)
--
-- serial -> int AUTO_INCREMENT
-- smallint -> tinyint (values sirf 0/1)
-- timestamp -> datetime (Postgres ka timestamp bhi literal storage hai; MySQL ka
--   TIMESTAMP session timezone me convert karta aur 2038 par ruk jaata hai)
-- bytea -> longblob
-- enum-derived text -> varchar(32) (MySQL me TEXT par na DEFAULT lagta hai na
--   bina prefix ke index; sabse lambi CHECK value 16 char ki hai)
-- BEFORE UPDATE trigger -> ON UPDATE CURRENT_TIMESTAMP
-- setval() -> ALTER TABLE ... AUTO_INCREMENT
...
CREATE TABLE cc_transactions (
  ...
  txn_type varchar(32) DEFAULT 'debit',
...
CREATE TABLE checklist_tasks (
  ...
  proof_replaced tinyint NOT NULL DEFAULT 0,
  proof_image text,
  frequency varchar(20) DEFAULT NULL,
  proof_video_id varchar(100) DEFAULT NULL,
  proof_video_mime varchar(50) DEFAULT NULL,
  proof_video_replaced tinyint NOT NULL DEFAULT 0,
  doer_remark text,
...
CREATE TABLE delegation_tasks (
  ...
  status varchar(32) DEFAULT 'pending',
  priority varchar(32) DEFAULT 'low',
  approval varchar(32) DEFAULT 'no',
  waiting_approval tinyint DEFAULT 0,
  awaiting_due_date tinyint DEFAULT 0,
  remarks text,
...
CREATE TABLE inventory_assignments (
  ...
  handover_status varchar(32) DEFAULT 'active',
  handover_notes text,
...
CREATE TABLE inventory_items (
  ...
  photo text,
  item_condition varchar(32) DEFAULT 'good',
...
CREATE TABLE payment_requests (
  ...
  reason text NOT NULL,
  status varchar(32) DEFAULT 'pending',
  payment_done tinyint DEFAULT 0,
...
CREATE TABLE tasks (
  ...
  purvi_notified tinyint DEFAULT 0,
  PRIMARY KEY (id),
...
CREATE TABLE users (
  ...
  extra_off text,
  exclude_from_reminder tinyint DEFAULT 0,
  extra_access text,
  ...
  staff_type varchar(32) NOT NULL DEFAULT 'office',
...
CREATE INDEX message_logs_idx_timestamp ON message_logs (datetime);
...
ALTER TABLE week_plans ADD COLUMN updated_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP;
